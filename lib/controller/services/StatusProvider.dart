...
class MediaProvider with ChangeNotifier {
  ...
  Future<void> fetchUserStatuses({int limit = 10, int offset = 0}) async {
    ...
    final String url =
        '${ApiURLs.baseUrl}${ApiURLs.get_user_status}$_loggedInUserId';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            "Failed to fetch user stories with status code: ${response.statusCode}");
      }
    } catch (e) {
  ...
  Future<void> fetchFollowerStories(BuildContext context,
      {int limit = 4, offset = 0}) async {
...
//debugger();
      if (response.statusCode == 200) {
        ...
        FollowerStoriesModel followerResponse =
            FollowerStoriesModel.fromJson(data);
          ...
          if (!_stories.any((story) => story.user!.id == user.id)) {
            _stories.add(userStatusModel.Stories(
              ...
              media: user.stories
                  ...
                  .map((media) => userStatusModel.Media(
                        id: media.id,
